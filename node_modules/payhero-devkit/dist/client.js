...
class PayHeroClient {
    ...
    async transactionStatus(reference) {
        const response = await this.client.get(`https://backend.payhero.co.ke/api/v2/transaction-status?reference=${reference}`);
        return response.data;
    }
    ...
    async servicewalletBalance(wallet_type) {
        try {
            const response = await this.client.get(`https://backend.payhero.co.ke/api/v2/wallets?wallet_type=${wallet_type}`);
            return response.data;
        }
        catch (error) {
            throw new Error(`Error fetching Service wallet balance: ${error.message}`);
        }
    }
    async paymentWalletBalance(walletChannelID) {
        if (!walletChannelID || typeof walletChannelID !== "number") {
            throw new Error("WalletChannelID must be a valid number");
        }
        try {
            const response = await this.client.get(`https://backend.payhero.co.ke/api/v2/payment_channels/${walletChannelID}`);
            return response.data;
        }
        catch (error) {
            throw new Error(`Failed to fetch Payment wallet balance: ${error.message}`);
        }
    }
    async serviceWalletTopup(data) {
        ...
        try {
            const response = await this.client.post("https://backend.payhero.co.ke/api/v2/topup", payload);
            return response.data;
        }
        catch (error) {
            throw new Error(`Error topping up serveice wallet: ${error.message}`);
        }
    }
    async walletWithdraw(data) {
        ...
        try {
            ...
            throw new Error(`Error processing withdrawal ${error}`);
        }
    }
}
exports.PayHeroClient = PayHeroClient;
