...
const client = new PayHeroClient({ authToken: process.env.AUTH_TOKEN });
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.status(503).json({ success: false, message: 'Service experiencing issues', error: error.message });
  }
});
