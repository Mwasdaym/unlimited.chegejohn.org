...
function updatePlanSummary(plan, category) {
    planSummary.innerHTML = `
        <div class="summary-item"><span>Category:</span> <span>${category}</span></div>
        <div class="summary-item"><span>Plan:</span> <span>${plan.name}</span></div>
        <div class="summary-item"><span>Duration:</span> <span>${plan.duration}</span></div>
        <div class="summary-item summary-total"><span>Total Amount:</span> <span>KES ${plan.price}</span></div>
    `;
}
...
function openDonationModal() {
    ...
    document.querySelectorAll('.donation-option').forEach(el => el.classList.remove('active'));
...
async function handlePaymentSubmission(e) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/initiate-payment`, {
            ...
            body: JSON.stringify({ planId: currentPlanId, phoneNumber, customerName, email })
        });
        const result = await response.json();
        if (result.success) {
            showPaymentResult(`
...
async function handleDonationSubmission(e) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/donate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ phoneNumber: donorPhone, amount: parseFloat(amount), customerName: donorName, message: donorMessage })
        });
        const result = await response.json();
        if (result.success) {
            showDonationResult(`
    ...
    const poll = async () => {
        ...
        try {
            showPaymentResult(`
                <div style="text-align: center;">
                    ...
                    <p style="margin-top: 0.5rem; color: var(--gray-600);">
                        Attempt ${attempts} of ${maxAttempts}<br> Please complete the payment on your phone
                    </p>
                </div>
            `, true, 'info');
            ...
            if (result.success && result.status === 'success') {
                ...
                `, true);
                setTimeout(() => { window.location.href = result.whatsappUrl; }, 3000);
                return;
            }
            ...
            if (result.success && result.status !== 'success' && attempts < maxAttempts) {
                ...
                showPaymentResult(`
                    <div style="text-align: center;">
                        ...
                        <p style="margin-top: 0.5rem; color: var(--gray-600);">Please contact support if payment was made</p>
                        <a href="https://wa.me/254743982206" style="display:inline-block; margin-top:1rem; padding:0.5rem 1rem; background:var(--success); color:white; text-decoration:none; border-radius:8px;">Contact Support</a>
                    </div>
                `, false);
            }
        } catch (error) {
            ...
            if (result.success && result.status === 'success') {
                showDonationResult(`
                    <div style="text-align: center;">
                        ...
                        <p style="margin-top: 0.5rem; color: var(--gray-600);">Your donation has been confirmed successfully</p>
                        <p style="margin-top: 1rem; font-size: 0.9rem;">We truly appreciate your support in helping us grow and improve our services.</p>
                    </div>
                `, true);
                return;
            }
            if (attempts < maxAttempts) setTimeout(poll, 10000);
            else showDonationResult(`
                <div style="text-align: center;">
                    <div style="font-size: 2rem; margin-bottom: 1rem;">⏰</div>
                    <strong>Donation Check Complete</strong>
                    <p style="margin-top: 0.5rem; color: var(--gray-600);">Thank you for your support! If you made a donation, it will be processed shortly.</p>
                </div>
            `, true);
        } catch (error) {
...
function formatDonorPhone(e) { formatPhoneNumber(e); }
...
async function checkServiceHealth() {
    try {
        ...
        const result = await response.json();
        if (!result.success) console.warn('Service health check warning:', result.message);
    } catch (error) {
